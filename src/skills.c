...
void do_gain(CHAR_DATA *ch, char *argument)
{
    ...
    if (!str_prefix(arg, "list"))
    {
        ...
        for (sn = 0; sn < MAX_SKILL; sn++)
        {
            ...
            if (!ch->pcdata->learned[sn] && skill_table[sn].rating[ch->class] > 0 && skill_table[sn].spell_fun == spell_null)
            {
                sprintf(buf, "%-18s %-5d ",
                        skill_table[sn].name,
...
void do_spells(CHAR_DATA *ch, char *argument)
{
    ...
    for (sn = 0; sn < MAX_SKILL; sn++)
    {
        ...
        if ((level = skill_table[sn].skill_level[ch->class]) < LEVEL_HERO + 1 && (fAll || level <= ch->level) && level >= min_lev && level <= max_lev && skill_table[sn].spell_fun != spell_null && ch->pcdata->learned[sn] > 0)
        {
            found = TRUE;
...
void list_group_costs(CHAR_DATA *ch)
{
    ...
    for (gn = 0; gn < MAX_GROUP; gn++)
    {
        ...
        if (!ch->gen_data->group_chosen[gn] && !ch->pcdata->group_known[gn] && group_table[gn].rating[ch->class] > 0)
        {
    ...
    for (sn = 0; sn < MAX_SKILL; sn++)
    {
        ...
        if (!ch->gen_data->skill_chosen[sn] && ch->pcdata->learned[sn] == 0 && skill_table[sn].spell_fun == spell_null && skill_table[sn].rating[ch->class] > 0)
        {
...
int exp_per_level(CHAR_DATA *ch, int points)
{
    ...
    if (points < 40)
        return 1000 * (pc_race_table[ch->race].class_mult[ch->class] ? pc_race_table[ch->race].class_mult[ch->class] /
                                                                           100
                                                                     : 1);

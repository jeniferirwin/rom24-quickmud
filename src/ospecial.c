...
bool ospec_password (CHAR_DATA * ch, OBJ_DATA * obj)
{
    ...
    if (source == NULL || obj == NULL || ch->fighting != NULL
        || !IS_AWAKE (ch) || IS_AFFECTED (ch, AFF_CHARM))
        return FALSE;
    ...
    if (!IS_TRUSTED (ch, ANGEL) && !IS_SET (obj->value[2], GATE_NOCURSE)
        && (IS_AFFECTED (ch, AFF_CURSE)
        || IS_SET (source->room_flags, ROOM_NO_RECALL)))
    {
    ...
    if (IS_SET (obj->value[2], GATE_RANDOM) || obj->value[3] == -1)
    {
        ...
        obj->value[3] = destination->vnum;    /* for record keeping :) */
             ...
             && (number_percent () < 5)) destination = get_random_room (ch);
    else
        destination = get_room_index (obj->value[3]);

    if (destination == NULL || destination == source
        || !can_see_room (ch, destination)
        || (room_is_private (destination) && !IS_TRUSTED (ch, IMPLEMENTOR)))
    {
    ...
    if (IS_NPC (ch) && IS_SET (ch->act, ACT_AGGRESSIVE)
        && IS_SET (destination->room_flags, ROOM_LAW))
    {

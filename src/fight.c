            ...
            int dam_type, bool show)
{
    ...
    */
    ...
    if (dam > 1 && ((IS_AFFECTED(victim, AFF_PROTECT_EVIL) && IS_EVIL(ch)) || (IS_AFFECTED(victim, AFF_PROTECT_GOOD) && IS_GOOD(ch)) || (IS_AFFECTED(victim, AFF_PROTECT_NEUTRAL) && IS_NEUTRAL(ch))))
    {
        dam -= dam / 4;
    }
    ...
    {
        if ((IS_SET(victim->act, ACT_WIMPY) && number_bits(2) == 0 && victim->hit < victim->max_hit / 5) || (IS_AFFECTED(victim, AFF_CHARM) && victim->master != NULL && victim->master->in_room != victim->in_room))
        {
...
void check_killer(CHAR_DATA *ch, CHAR_DATA *victim)
{
     ...
     */
    if (IS_NPC(ch) || ch == victim || ch->level >= LEVEL_IMMORTAL || !is_clan(ch) || IS_SET(ch->act, PLR_KILLER) || ch->fighting == victim)
        return;
...
void do_bash(CHAR_DATA *ch, char *argument)
{
    ...
    { /*
        act("#w$n tries to bash you, but you dodge it.#w",ch,NULL,victim,TO_VICT);
        act("#w$N dodges your bash, you fall flat on your face.#w",ch,NULL,victim,TO_CHAR);
    ...
    else
    {
        ...
        act("#wYou fall flat on your face!#w", ch, NULL, victim, TO_CHAR);
        act("#w$n falls flat on $s face.#w", ch, NULL, victim, TO_NOTVICT);
        act("#wYou evade $n's bash, causing $m to fall flat on $s face.#w",
            ch, NULL, victim, TO_VICT);
...
void do_dirt(CHAR_DATA *ch, char *argument)
{
    ...
    if ((chance = get_skill(ch, gsn_dirt)) == 0 || (IS_NPC(ch) && !IS_SET(ch->off_flags, OFF_KICK_DIRT)) || (!IS_NPC(ch) && ch->level < skill_table[gsn_dirt].skill_level[ch->class]))
    {
    ...
    if (number_percent() < chance)
    {
        ...
        act("#w$n is blinded by the dirt in $s eyes!#w", victim, NULL, NULL,
            TO_ROOM);
        act("#w$n kicks dirt in your eyes!#w", ch, NULL, victim, TO_VICT);
        damage(ch, victim, number_range(2, 5), gsn_dirt, DAM_NONE, FALSE);
        send_to_char("#wYou can't see a thing!#w\n\r", victim);
...
void do_trip(CHAR_DATA *ch, char *argument)
{
    ...
    if ((chance = get_skill(ch, gsn_trip)) == 0 || (IS_NPC(ch) && !IS_SET(ch->off_flags, OFF_TRIP)) || (!IS_NPC(ch) && ch->level < skill_table[gsn_trip].skill_level[ch->class]))
    {
    ...
    if (victim == ch)
    {
        send_to_char("#wYou fall flat on your face!#w\n\r", ch);
        WAIT_STATE(ch, 2 * skill_table[gsn_trip].beats);
        act("#w$n trips over $s own feet!#w", ch, NULL, NULL, TO_ROOM);
        return;
    }
...
void do_flee(CHAR_DATA *ch, char *argument)
{
    ...
    for (attempt = 0; attempt < 6; attempt++)
    {
        ...
        door = number_door();
        if ((pexit = was_in->exit[door]) == 0 || pexit->u1.to_room == NULL || IS_SET(pexit->exit_info, EX_CLOSED) || number_range(0, ch->daze) != 0 || (IS_NPC(ch) && IS_SET(pexit->u1.to_room->room_flags, ROOM_NO_MOB)))
            continue;
...
void do_disarm(CHAR_DATA *ch, char *argument)
{
    ...
    {
        WAIT_STATE(ch, skill_table[gsn_disarm].beats);
        act("#wYou fail to disarm $N.#w", ch, NULL, victim, TO_CHAR);
        act("#w$n tries to disarm you, but fails.#w", ch, NULL, victim,
            TO_VICT);
        act("#w$n tries to disarm $N, but fails.#w", ch, NULL, victim,
            TO_NOTVICT);
...
void do_slay(CHAR_DATA *ch, char *argument)
{
    ...
    act("#wYou slay $M in cold blood!#w", ch, NULL, victim, TO_CHAR);
    act("#w$n slays you in cold blood!#w", ch, NULL, victim, TO_VICT);
    act("#w$n slays $N in cold blood!#w", ch, NULL, victim, TO_NOTVICT);
    raw_kill(victim);
    return;
}

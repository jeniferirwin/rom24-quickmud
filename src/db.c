...
void load_old_obj(FILE *fp)
{
    ...
    for (;;)
    {
        ...
        {
            if (is_name("two", pObjIndex->name) || is_name("two-handed", pObjIndex->name) || is_name("claymore", pObjIndex->name))
...
void load_resets(FILE *fp)
{
    ...
    for (;;)
    {
        ...
        case 'D':
            ...
            if (pReset->arg2 < 0 || pReset->arg2 >= MAX_DIR || !pRoomIndex || !(pexit = pRoomIndex->exit[pReset->arg2]) || !IS_SET(pexit->rs_flags, EX_ISDOOR))
            {
             ...
             pRoomIndex != NULL; pRoomIndex = pRoomIndex->next)
        {
                 ...
                 pReset = pReset->next)
            {
                ...
                case 'P':
                    ...
                    if (iLastObj == NULL)
                    {
                        bugf("fix_exits : reset in room %d with iLastObj NULL",
                             pRoomIndex->vnum);
                        exit(1);
                    }
                    break;
...
void reset_room(ROOM_INDEX_DATA *pRoom)
{
    ...
    for (pReset = pRoom->reset_first; pReset != NULL; pReset = pReset->next)
    {
        ...
        case 'P':
            ...
            if (pRoom->area->nplayer > 0 || (LastObj = get_obj_type(pObjToIndex)) == NULL || (LastObj->in_room == NULL && !last) || (pObjIndex->count >= limit /* && number_range(0,4) != 0 */) || (count = count_obj_list(pObjIndex, LastObj->contains)) > pReset->arg4)
            {
                last = FALSE;
                break;
            }
        ...
        case '~':
            plast++;
            {
                ...
                for (pHash = string_hash[iHash]; pHash; pHash = pHashPrev)
                {
                    ...
                    if (top_string[sizeof(char *)] == pHash[0] && !strcmp(top_string + sizeof(char *) + 1,
                                                                          pHash + 1))
                        return pHash;
                }

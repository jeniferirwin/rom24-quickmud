...
void gain_exp(CHAR_DATA *ch, int gain)
{
    ...
    {
        send_to_char("#GYou raise a level!!  #w", ch);
        ch->level += 1;
...
void char_update(void)
{
    ...
    for (ch = char_list; ch != NULL; ch = ch_next)
    {
        ...
        if (ch->position >= POS_STUNNED)
        {
            ...
            if (IS_NPC(ch) && ch->zone != NULL && ch->zone != ch->in_room->area && ch->desc == NULL && ch->fighting == NULL && !IS_AFFECTED(ch, AFF_CHARM) && number_percent() < 5)
            {

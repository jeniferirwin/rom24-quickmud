...
void do_cast(CHAR_DATA *ch, char *argument)
{
    ...
    if ((sn = find_spell(ch, arg1)) < 1 || skill_table[sn].spell_fun == spell_null || (!IS_NPC(ch) && (ch->level < skill_table[sn].skill_level[ch->class] || ch->pcdata->learned[sn] == 0)))
    {
                        ...
                        int target)
{
    ...
    if (IS_AFFECTED(victim, AFF_CHARM) || IS_AFFECTED(ch, AFF_CHARM) || level < victim->level || IS_SET(victim->imm_flags, IMM_CHARM) || saves_spell(level, victim, DAM_CHARM))
        return;
                        ...
                        int target)
{
    ...
    if (IS_AFFECTED(victim, AFF_DARK_FAVOR) && !saves_dispel(level, victim->level, -1) && !is_affected(victim, skill_lookup("dark favor")))
    {
    ...
    if (IS_AFFECTED(victim, AFF_SANCTUARY) && !saves_dispel(level, victim->level, -1) && !is_affected(victim, skill_lookup("sanctuary")))
    {
                      ...
                      int target)
{
    ...
    if (!saves_spell(level + 2, victim, DAM_FIRE) && !IS_SET(victim->imm_flags, IMM_FIRE))
    {
             ...
             obj_lose != NULL; obj_lose = obj_next)
        {
            ...
            if (number_range(1, 2 * level) > obj_lose->level && !saves_spell(level, victim, DAM_FIRE) && !IS_OBJ_STAT(obj_lose, ITEM_NONMETAL) && !IS_OBJ_STAT(obj_lose, ITEM_BURN_PROOF))
            {
                ...
                case ITEM_WEAPON:
                    ...
                    { /* try to drop it */
                        ...
                        if (can_drop_obj(victim, obj_lose) && remove_obj(victim, obj_lose->wear_loc,
                                                                         TRUE))
                        {
                           ...
                           int target)
{
    ...
    if (IS_AFFECTED(victim, AFF_PROTECT_EVIL) || IS_AFFECTED(victim, AFF_PROTECT_GOOD) || IS_AFFECTED(victim, AFF_PROTECT_NEUTRAL))
    {
        if (victim == ch)
                           ...
                           int target)
{
    ...
    if (IS_AFFECTED(victim, AFF_PROTECT_GOOD) || IS_AFFECTED(victim, AFF_PROTECT_EVIL) || IS_AFFECTED(victim, AFF_PROTECT_NEUTRAL))
    {
        if (victim == ch)
            send_to_char("You are already protected.\n\r", ch);
        else
...
void spell_sleep(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    if (IS_AFFECTED(victim, AFF_SLEEP) || (IS_NPC(victim) && IS_SET(victim->act, ACT_UNDEAD)) || (level + 2) < victim->level || saves_spell(level - 4, victim, DAM_CHARM))
        return;

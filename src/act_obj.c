...
void do_put(CHAR_DATA *ch, char *argument)
{
    ...
    else
    {
        ...
        for (obj = ch->carrying; obj != NULL; obj = obj_next)
        {
            ...
            if ((arg1[3] == '\0' || is_name(&arg1[4], obj->name)) && can_see_obj(ch, obj) && WEIGHT_MULT(obj) == 100 && obj->wear_loc == WEAR_NONE && obj != container && can_drop_obj(ch, obj) && get_obj_weight(obj) + get_true_weight(container) <= (container->value[0] * 10) && get_obj_weight(obj) < (container->value[3] * 10))
            {
...
void wear_obj(CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace)
{
    ...
    if (CAN_WEAR(obj, ITEM_WIELD))
    {
        ...
        if (!IS_NPC(ch) && ch->size < SIZE_LARGE && IS_WEAPON_STAT(obj, WEAPON_TWO_HANDS) && get_eq_char(ch, WEAR_SHIELD) != NULL)
        {
...
void do_steal(CHAR_DATA *ch, char *argument)
{
    ...
    if (((ch->level + 7 < victim->level || ch->level - 7 > victim->level) && !IS_NPC(victim) && !IS_NPC(ch)) || (!IS_NPC(ch) && percent > get_skill(ch, gsn_steal)) || (!IS_NPC(ch) && !is_clan(ch)))
    {
        /*
         * Failure.
         */

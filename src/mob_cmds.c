...
void do_mpvforce(CHAR_DATA *ch, char *argument)
{
    ...
    for (victim = char_list; victim; victim = victim_next)
    {
        ...
        if (IS_NPC(victim) && victim->pIndexData->vnum == vnum && ch != victim && victim->fighting == NULL)
            interpret(victim, argument);
    }
    return;
}
...
void do_mpflee(CHAR_DATA *ch, char *argument)
{
    ...
    {
        door = number_door();
        if ((pexit = was_in->exit[door]) == 0 || pexit->u1.to_room == NULL || IS_SET(pexit->exit_info, EX_CLOSED) || (IS_NPC(ch) && IS_SET(pexit->u1.to_room->room_flags, ROOM_NO_MOB)))
            continue;

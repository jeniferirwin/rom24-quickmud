...
void show_char_to_char_0(CHAR_DATA *victim, CHAR_DATA *ch)
{
    ...
    if (IS_AFFECTED(victim, AFF_INVISIBLE) || victim->invis_level >= LEVEL_HERO || IS_AFFECTED(victim, AFF_HIDE) || IS_AFFECTED(victim, AFF_CHARM) || IS_AFFECTED(victim, AFF_PASS_DOOR) || IS_AFFECTED(victim, AFF_FAERIE_FIRE) || (IS_EVIL(victim) && IS_AFFECTED(ch, AFF_DETECT_EVIL)) || (IS_GOOD(victim) && IS_AFFECTED(ch, AFF_DETECT_GOOD)) || IS_AFFECTED(victim, AFF_SANCTUARY) || IS_AFFECTED(victim, AFF_DARK_FAVOR))
    {
        brackets = 1;
    }
    if (brackets == 1)
    ...
    for (d = descriptor_list; d != NULL; d = d->next)
    {
        ...
        if (!str_prefix(arg, wch->name))
        {
            ...
            sprintf(buf, "[%2d %6s %s] %s%s%s%s%s%s%s%s\n\r",
                    ...
                    clan_table[wch->clan].who_name, IS_SET(wch->comm, COMM_AFK) ? "[AFK] " : "", IS_SET(wch->act, PLR_KILLER) ? "(KILLER) " : "",
...
void do_who(CHAR_DATA *ch, char *argument)
{
    ...
    for (d = descriptor_list; d != NULL; d = d->next)
    {
        ...
        if (wch->level < iLevelLower || wch->level > iLevelUpper || (fImmortalOnly && wch->level < LEVEL_IMMORTAL) || (fClassRestrict && !rgfClass[wch->class]) || (fRaceRestrict && !rgfRace[wch->race]) || (fClan && !is_clan(wch)) || (fClanRestrict && !rgfClan[wch->clan]))
            continue;
...
void do_where(CHAR_DATA *ch, char *argument)
{
    ...
    if (arg[0] == '\0')
    {
        ...
        {
            if (d->connected == CON_PLAYING && (victim = d->character) != NULL && !IS_NPC(victim) && victim->in_room != NULL && !IS_SET(victim->in_room->room_flags, ROOM_NOWHERE) && (is_room_owner(ch, victim->in_room) || !room_is_private(victim->in_room)) && victim->in_room->area == ch->in_room->area && can_see(ch, victim))
            {
                found = TRUE;
                sprintf(buf, "%-28s %s\n\r",
